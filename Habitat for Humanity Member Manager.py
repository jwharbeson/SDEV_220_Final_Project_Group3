...
class Member:
    def __init__(self, first_name, last_name, address, contact_phone, birthday, join_date):
...
class Communication:
    ...
    def send_messages(self):
        for msg in self.messages:
            ...
            print(f"Sending '{message}' to {member.first_name} {member.last_name} ({member.contact_phone})")
...
class MemberApp:
    def __init__(self, root):
        ...
        self.first_name_entry = tk.Entry(root, width=entry_width)
        ...
        self.last_name_entry = tk.Entry(root, width=entry_width)
        ...
        self.address_entry = tk.Entry(root, width=entry_width)
        self.address_entry.grid(row=2, column=1, padx=10, pady=5)

        # Primary Contact Phone #
        self.contact_label = tk.Label(root, text="Primary Contact Phone #")
        self.contact_label.grid(row=3, column=0, sticky="w", padx=10, pady=5)
        self.contact_entry = tk.Entry(root, width=entry_width)
        self.contact_entry.grid(row=3, column=1, padx=10, pady=5)

        # Birthday
        self.birthday_label = tk.Label(root, text="Birthday (MM-DD-YYYY)")
        self.birthday_label.grid(row=4, column=0, sticky="w", padx=10, pady=5)
        self.birthday_entry = tk.Entry(root, width=entry_width)
        self.birthday_entry.grid(row=4, column=1, padx=10, pady=5)

        # Join Date
        self.join_date_label = tk.Label(root, text="Join Date (MM-DD-YYYY)")
        self.join_date_label.grid(row=5, column=0, sticky="w", padx=10, pady=5)
        self.join_date_entry = tk.Entry(root, width=entry_width)
        self.join_date_entry.grid(row=5, column=1, padx=10, pady=5)

        # Add Member button
        self.add_button = tk.Button(root, text="Add Member", command=self.add_member)
        self.add_button.grid(row=6, column=1, padx=10, pady=10)

        # List to display members
        self.members_listbox = tk.Listbox(root, width=entry_width)
        self.members_listbox.grid(row=7, column=0, columnspan=2, padx=10, pady=10)

        # Assign Role button
        self.assign_role_button = tk.Button(root, text="Assign Role", command=self.assign_role)
        self.assign_role_button.grid(row=8, column=1, padx=10, pady=10)

        # View Role Members button
        self.view_roles_button = tk.Button(root, text="View Role Members", command=self.view_role_members)
        self.view_roles_button.grid(row=9, column=1, padx=10, pady=10)

        # Role Entry (Moved below View Role Members button)
        self.role_label = tk.Label(root, text="Role")
        self.role_label.grid(row=10, column=0, sticky="w", padx=10, pady=5)
        self.role_entry = tk.Entry(root, width=entry_width)
        self.role_entry.grid(row=10, column=1, padx=10, pady=5)

        # Schedule Message button
        self.message_button = tk.Button(root, text="Schedule Message", command=self.schedule_message)
        self.message_button.grid(row=11, column=1, padx=10, pady=10)

        # Send Message button
        self.send_message_button = tk.Button(root, text="Send Messages", command=self.send_messages)
        self.send_message_button.grid(row=12, column=1, padx=10, pady=10)

        # Custom message entry
        self.custom_message_label = tk.Label(root, text="Custom Message")
        self.custom_message_label.grid(row=13, column=0, sticky="w", padx=10, pady=5)
        self.custom_message_entry = tk.Entry(root, width=entry_width)
        self.custom_message_entry.grid(row=13, column=1, padx=10, pady=5)

        # Text box to display role members
        self.role_members_textbox = tk.Text(root, width=entry_width, height=10)
        self.role_members_textbox.grid(row=14, column=0, columnspan=2, padx=10, pady=10)
    ...
    def add_member(self):
        ...
        member = Member(first_name, last_name, address, contact_phone, birthday, join_date)
        members.append(member)

        # Update the display list
        self.members_listbox.insert(tk.END, f"{first_name} {last_name} - {contact_phone}")
